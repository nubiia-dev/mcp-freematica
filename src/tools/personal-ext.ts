...
export function registerPersonalExtTools(
  ...
  server.tool(
    ...
    [
      ...
      'NOTA: este endpoint no tiene GET singular, por lo que NO se hace',
      'fetch+merge. Envía todos los campos relevantes: el servidor puede no',
      'realizar actualización parcial y sobrescribir con lo enviado.',
    ].join('\n'),
    UpdatePersonaAdicionalShape,
    { readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    async ({ idReg, ...rest }): Promise<CallToolResult> => {
